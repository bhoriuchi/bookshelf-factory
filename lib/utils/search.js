...
module.exports = function(config) {
	...
	var specific  = config.utils.sql.specific;
	...
	function getSearchable(parentSchema, childSchema) {
		...
		if (childSchema) {
			...
			childSearchable = _.keys(
				_.pick(childFields, function(c) {
					return !c.hasOwnProperty(_SRCH.searchable) || c.searchable;
				})
			);
		}
		...
		parentSearchable = _.keys(
			_.pick(parentFields, function(p) {
				return !p.hasOwnProperty(_SRCH.searchable) || p.searchable;
			})
		);
	...
	function getColumnConcat(table, schema, searchable) {
		...
		// make search-able into an array
	...
	function getRegexSQL(regex, compareTo) {
		...
		if (typeof (specific[dbType]) === _JTYP.object &&
				specific[dbType].hasOwnProperty(_SRCH.regex)) {
