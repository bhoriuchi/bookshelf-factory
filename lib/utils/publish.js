					...
					return result.save({ current_version: new_version }, saveOpts).then(function(result) {
						...
						if (!result) {
							throw u.newErr(
								...
								['Could not update the current_version field on ' + tableName + ' with ID ' + id]
							);
						}
								...
								.then(function(result) {
									...
									if (!result) {
										throw u.newErr(
											...
											_ERR.NOT_FOUND.detail,
											_ERR.NOT_FOUND.code,
											['A draft version for ' + tableName + ' with ID ' + id + ' was not found']
										);
									}
								...
								.then(function(results) {
									...
									if (!results) {
										throw u.newErr(
											...
											['The draft for ' + tableName + ' with ID ' + id + ' was not promoted']
										);
									}
									
									// create a new draft by cloning the current
									return cModel.forge().clone(results.id, {transacting: t}).end();
