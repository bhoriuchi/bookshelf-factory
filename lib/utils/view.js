...
module.exports = function(config) {
	...
	function filter(view, table, schema, prefix, circular) {
			...
			_.forEach(tableSchema, function(col, colName) {
				if (_.contains(col.views, view) || _.contains(col.views, '*')) {
					...
					if (relation) {
						...
						if (!_.contains(circular, col[relation])) {
							newCircular = _.cloneDeep(circular);
							newCircular.push(col[relation]);
							list = _.union(list, filter(view, col[relation], schema, colName, newCircular));
						}
					}
				}
			});
				...
				_.forEach(schema[tableSchema._versioned.model], function(col, colName) {
					if (_.contains(col.views, view) || _.contains(col.views, '*')) {
						...
						if (relation) {
							...
							if (!_.contains(circular, nextTable)) {
								newCircular = _.cloneDeep(circular);
								newCircular.push(nextTable);
								list = _.union(list, filter(view, nextTable, schema, colName, newCircular));
							}
						}
					}
				});
