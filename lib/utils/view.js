...
module.exports = function(config) {
	...
	// TODO use this function to optimize which relations need to be retrieved when using
	// get caching
	
	// function to build a view reference
	/*
	function viewReference(tableName, models, view, depth, maxDepth) {
		...
		view = (Array.isArray(view)) ? view :
			   utils.view.filter(view, tableName, models._schema, '');
		
		// look through the view
		_.forEach(view, function(path) {
			
			// set up limiting params
			var circular = [tableName];
			var depth    = 0;
			
			// get a list of fields
			var fields = path.replace('!', '').split('.');
