...
module.exports = function(config) {
			...
			_.forEach(tableSchema, function(colSchema, colName) {
				...
				if (relation.length > 0) {
					...
					if (relation === CONST.hasOne || relation === CONST.hasMany) {
						...
						foreignKey = colSchema[CONST.foreignKey] || colName + '_' + pk;
	...
	function selectModel(oTable, oId, models, relData, result) {
		...
		if (isVersioned(models._schema[relData.targetTableName]) &&
				result.hasOwnProperty('attributes') &&
				_.contains(Object.keys(result.attributes), 'parent_id')) {
			mId   = result.attributes.parent_id;
			mName = models._schema[relData.targetTableName]._managed.model;
		}
