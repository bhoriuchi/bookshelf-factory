	...
	function makeVersion(schema) {
		...
		_.forEach(schema, function(tableSchema, tableName) {
			...
			if (_.keys(versionedCols).length > 0 && !tableSchema._versioned && !tableSchema._managed) {
				...
				schema[verTableName].published    = {type: _SCMR.type.boolean, defaultTo: false, views:['*']};
				schema[verTableName].version      = {type: _SCMR.type.integer, defaultTo: 0, views:['*']};
				schema[verTableName].valid_from   = {type: _SCMR.type.bigInteger, nullable: true, views:['*']};
				schema[verTableName].valid_to     = {type: _SCMR.type.bigInteger, nullable: true, views:['*']};
				...
				schema[tableName].active          = {type: _SCMR.type.boolean, defaultTo: true, views:['*']};
				schema[tableName].use_current     = {type: _SCMR.type.boolean, defaultTo: true, views:['*']};
				schema[tableName].current_version = {type: _SCMR.type.integer, defaultTo: 0, views:['*']};
