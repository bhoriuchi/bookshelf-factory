...
module.exports = function(config) {
	...
	function getPrimarySchema(col) {
		...
		if (_.has(col, _SCMR.options.type)) {
			schema[_SCMR.options.type] = col[_SCMR.options.type];
		}
		
		if (_.has(col, _SCMR.options.size)) {
	...
	function getColumns(schema) {
		...
		if (schema && typeof(schema) === _JTYP.object) {
			return _.pick(schema, function(colSchema) {
				return _.has(colSchema, _SCMR.options.type) &&
				(!_.has(colSchema, _SCMR.options.ignore) || !colSchema.ignore);
			});
		}
		return {};
	}
	...
	// function to get the relation in the current column
	function getRelation(colSchema) {
	...
	function connectRelations(schema) {
		...
		_.forEach(schema, function(tableSchema, tableName) {
			_.forEach(tableSchema, function(colSchema, colName) {
				...
				// find relations
				var relation = getRelation(colSchema);
				
				// look for a connectRelation property
				if (relation &&
						_.has(colSchema, _SCMA.connectRelation) &&
					...
					var connectRelation = _.intersection(_.keys(connectSchema), relations);
	...
	function makeVersion(schema) {
		...
		_.forEach(schema, function(tableSchema, tableName) {
			...
			if (_.keys(versionedCols).length > 0 && !tableSchema._versioned && !tableSchema._managed) {
				
				// add the tables to process
				toProcess.push(tableName);
				toProcess.push(tableName + _SCMA.versionSuffix);
				...
				var verTableName                  = tableName + _SCMA.versionSuffix;
				schema[verTableName]              = schema[verTableName] || {};
				schema[verTableName].id           = parentSchemaId;
				schema[verTableName].parent_id    = _.merge({index: true}, parentSchemaParent);
				schema[verTableName].published    = {type: _SCMR.type.boolean, index: true, defaultTo: false, views:['*']};
				schema[verTableName].version      = {type: _SCMR.type.integer, index: true, defaultTo: 0, views:['*']};
				schema[verTableName].valid_from   = {type: _SCMR.type.bigInteger, index: true, nullable: true, views:['*']};
				schema[verTableName].valid_to     = {type: _SCMR.type.bigInteger, index: true, nullable: true, views:['*']};
				schema[verTableName].change_notes = {type: _SCMR.type.string, size: 500, nullable: true, views:['*']};
				
				// if there are temporal columns, move them over to the temporal object
				_.forEach(versionedCols, function(colSchema, colName) {
					
					if (_.keys(colSchema).length > 0) {
						
						// first check for unique and change it to groupUnique
						if (colSchema.unique === true) {
							delete schema[tableName][colName].unique;
							schema[tableName][colName].groupUnique = _VER.child.parent_id;
						}
				...
				schema[tableName].active          = {type: _SCMR.type.boolean, index: true, defaultTo: true, views:['*']};
				schema[tableName].use_current     = {type: _SCMR.type.boolean, index: true, defaultTo: true, views:['*']};
				schema[tableName].current_version = {type: _SCMR.type.integer, index: true, defaultTo: 0, views:['*']};
			...
			_.forEach(tableSchema, function(colSchema, colName) {
				...
				if (tableSchema._managed && _.contains(toProcess, tableName)) {
					...
					if (relation && schema[colSchema[relation]] &&
						...
						schema[tableName][colName][relation] = colSchema[relation] + _SCMA.versionSuffix;
					}
				}
			});
		});
		...
		_.forEach(schema, function(tableSchema, tableName) {
			_.forEach(tableSchema, function(colSchema, colName) {
				...
				if (relation === _REL.hasOne || relation === _REL.hasMany || relation === _REL.belongsTo) {

					if (_.has(schema[colSchema[relation]], '_managed.model') && _.has(tableSchema, '_managed.model')) {
						
						// get the current foreign key
						var fk     = colSchema.foreignKey;
						var newFk  = fk;
						
						// create regex to match the current foreignKey
						var tblRx = new RegExp('^fk_' + tableSchema._managed.model + '_');
						var relRx = new RegExp('^fk_' + schema[colSchema[relation]]._managed.model + '_');
						
						// if the current table, replace with the versioned table
						if (fk.match(tblRx)) {
							newFk = fk.replace(tblRx, 'fk_' + tableName + '_');
						}
						else if (fk.match(relRx)) {
							newFk = fk.replace(relRx, 'fk_' + colSchema[relation] + '_');
						}

						
						// set the new foreign key
						schema[tableName][colName].foreignKey = newFk;
						

						if (relation === _REL.belongsTo && _.has(schema[tableName], fk)) {
							schema[tableName][newFk] = _.cloneDeep(schema[tableName][fk]);
							delete schema[tableName][fk];
						}
						else if (relation !== _REL.belongsTo && _.has(schema[colSchema[relation]], fk)) {
							schema[colSchema[relation]][newFk] = _.cloneDeep(schema[colSchema[relation]][fk]);
							delete schema[colSchema[relation]][fk];
						}
					}
				}
				// look for belongs to many tables and update the table name
					...
					if ((tableSchema._managed || relTableSchema._managed) && schema[colSchema.junction]) {
						...
						var junction = _SCMA.junctionPrefix + masterField + '_' + junctionTables.join('_');
			...
			_.forEach(tableSchema, function(colSchema, colName) {
				...
				if (relation) {
					...
					if (relation === _REL.hasOne || relation === _REL.hasMany) {
						...
						if (!_.has(schema[relTableName], colSchema.foreignKey)) {
					...
					// check for belongsTo relations
						...
						if (colSchema.connectRelation && relSchema &&
								_.has(relSchema, colSchema.connectRelation)) {
						...
						if (!_.has(schema[tableName], colSchema.foreignKey)) {
					...
					// check for belongsToMany relation
						...
						if (colSchema.connectRelation && relSchema) {
							...
							colSchema.junction = colSchema.junction ||
							                     _SCMA.junctionPrefix + masterColSchema.connectRelation +
							...
							colSchema.junction = colSchema.junction ||
							                     _SCMA.junctionPrefix + colName + '_' +
						...
						if (!_.has(schema,colSchema.junction)) {
							schema[colSchema.junction] = {};
						}
						
						// check for foreign key, create if doesn't exist
						if (!_.has(schema[colSchema.junction], colSchema.foreignKey)) {
						...
						if (!_.has(schema[colSchema.junction], colSchema.otherKey)) {
	...
	function selectModel(originalTable, models, relObj) {
		...
		if (parent) {
			...
			id        = relObj.attributes.parent_id;
		}
