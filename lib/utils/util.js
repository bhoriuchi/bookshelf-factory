...
module.exports = function(config) {
	...
	function resolveDate(obj) {
		...
		// check for string, most common
		if (obj && typeof(obj) === _JTYP.string) {
			
			// check that the string is not a number, which should
			// indicate that it is a formatted date string that
			// moment can hopefully parse into a number
			if (isNaN(obj)) {
				return moment(obj).valueOf();
			}
			
			// otherwise it is a number, so convert the string to
			// an integer and run it though moment (probably redundant)
			else {
				return moment(parseInt(obj, 10)).valueOf();
			}
			
		}
		// check for number, UNIX time
		else if (obj && typeof (obj) === _JTYP.number) {
			return moment(obj).valueOf();
		}
		// check for a date object
		else if (obj && typeof (obj) === _JTYP.object &&
				typeof(obj.getDate) === _JTYP.funct) {
			return obj.valueOf();
		}
		else {
			return null;
