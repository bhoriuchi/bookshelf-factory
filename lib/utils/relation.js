...
module.exports = function(config) {
	...
	function syncRelations(models, relations, model, t) {
		...
		return promise.each(relations, function(relation) {
			...
			if (rel && typeof (rel) === _JTYP.object) {
				...
				.then(function(saved) {
					...
					return utils.validate.verifyEntities(models, pTableName, ids, t).then(function(invalid) {
						...
						if (invalid.length === 0) {
							...
							if (dat.type === _REL.belongsToMany) {
								...
								// check for temporal
								if (_.has(models._schema[dat.targetTableName], _SCMA._managed)) {
