	...
	function syncRelations(relations, model, t) {
		...
		return promise.each(relations, function(relation) {
			...
			if (typeof (rel) === 'object' && rel !== null) {
				...
				var where       = {};
				var save        = {};
				var idAttr      = dat.targetIdAttribute;
				var tableName   = dat.targetTableName;
				var foreignKey  = dat.foreignKey;
				var tableSchema = models._schema[tableName];
				var versioned   = tableSchema.hasOwnProperty('_managed');//tableSchema.hasOwnProperty(foreignKey) ? false : true;
				var Model       = models[tableName];
				var ver         = versioned ? 'version = 0' : '1 = 1';
