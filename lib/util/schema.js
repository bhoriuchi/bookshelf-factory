...
module.exports = function(config) {
	...
	function infect(schema) {
		...
		_.forEach(schema, function(tableSchema, tableName) {
			_.forEach(tableSchema, function(colSchema, colName) {
				...
				// get the primary key and has relationships
				...
				if (relation.length > 0) {
					...
					if (relation === CONST.hasOne || relation === CONST.hasMany) {
						...
						if (_.has(colSchema, 'versioned') && colSchema.versioned &&
							...
							//schema[tableName][colName][relation] = relTable + 'version';
	...
	// function to find if the schema contains any circular references that would cause nested relationship issues
	function hasCircular(schema) {
	...
	function getCircular(schema, table, parentList) {
		
		if (_.contains(parentList, table)) {
			return true;
		}
		
		
		_.forEach(schema[table], function(colSchema, colName) {
			
			var relType = _.intersection(_.keys(colSchema), relations);
			
			if (relType.length > 0) {
				
				var tableRef = colSchema[relType[0]];
