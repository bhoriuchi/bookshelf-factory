...
module.exports = function(config) {
		...
		if (tableSchema.hasOwnProperty('_managed')) {
			...
			_.forEach(tableSchema, function(colSchema, colName) {
				...
				// check for auto incrementing uuid columns on initial save
					...
					payload[colName] = getServerTime().uuid;
				}
	...
	function getTime(callback) {
		...
		// check if the time source is a server
		if (config.timesource.hasOwnProperty('server') && config.timesource.server !== '') {
			
			var port = config.timesource.port || 123;
			
			// attempt to get the network time, default to server time if failure
			ntp.getNetworkTime(config.timesource.server, port, function(err, date) {
				if (err) {
					console.log('ERROR: failed to get network time from ' +
							config.timesource.server + ' on port ' + port +
							', defaulting to server time');
		...
		// otherwise return the server time and uuid
		else {
			callback(getServerTime());
		}
