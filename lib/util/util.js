...
module.exports = function(config) {
	...
	function checkPayload(table, payload, method, managedCheck, sysTime) {
		...
		var versioned    = false;
		var pl           = {};
		var useUuid      = tableSchema.hasOwnProperty('_managed') ? sysTime.uuid2 : sysTime.uuid1;
	...
	function getTime(knex) {
		
		// try to the uuids and date from the database
		return knex.raw('SELECT UUID() AS uuid1, UUID() AS uuid2, UTC_TIMESTAMP() AS date')
		.then(function(results) {
