	...
	function checkPayload(table, payload, method) {
		...
		if (tableSchema.hasOwnProperty('_managed')) {
			...
			if (tableSchema.hasOwnProperty('_versioned')) {
				...
				// split the payload into its tables
				var pl = [];
				...
				pl.push({
					...
					payload: _.omit(_.mapValues(schema[table], function(value, key) {
						return _.has(payload, key) ? payload[key] : null;
					}), function(value, key) {
						return _.has(constants.versioned.parent, key);
					})
				});
				...
				pl.push({
					...
					payload: _.omit(_.mapValues(schema[tableSchema._versioned.model], function(value, key) {
						...
						return _.has(constants.versioned.child, key);
					})
				});
