...
module.exports = function(config) {
	 ...
	 * @name clone
	 * @function
	 * @description - Creates a new copy of the factory model passed to it and initializes all of the factory specific variables. You will find this necessary when performing multiple separate transactions on the same model. Since the transaction is kept in the models _var hash, when it has been completed it is not cleaned up in order to support chaining methods. In general it is best practice to use this method on each use of the model.
	 * @param {Object} model - A model created by factory.create(), or Bookshelf.Model.Extend().
	 * @returns {Object}
	 */
	function clone(model) {
		
		if (typeof(model.extend) === _JTYP.funct) {
			return model.extend({
				_var: null,
				results: null
			});
