...
module.exports = function(config) {
	...
	function create(schema) {
		...
		// update the mods to include the rest of the modules
		mods = _.merge(mods, {
			util: util,
			schema: schema,
			tables: tables,
		...
		// create a methods object that will be used to extend the prototype
		...
		for(var i = 0; i < tables.length; i++) {
			...
			// create model in the global factory store
