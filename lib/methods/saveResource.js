	...
	return function(obj, fetchOpts, jsonOpts) {
		...
		return _ts.then(function(sysTime) {
			...
			if (pkg.valid) {
				...
				if (fetchOpts.hasOwnProperty('_transaction')) {
					_op = utils.save(pkg.payloads, models)(fetchOpts._transaction);
				}
				
				// otherwise create a new transaction and call save
				else {
					_op = Bookshelf.transaction(utils.save(pkg.payloads, models));
				}
				
				
				// execute operation and get the results
				return _op.then(function(model) {
					...
					if (model && Array.isArray(model) && model.length > 0) {
						...
						return promise.map(model, function(m) {
							...
							if (typeof (pretty) === 'object' && pretty.enabled) {
								return m.view(keep).pretty(pretty.spacing).getResource(m.attributes[idAttr], fetchOpts);
							}
							else {
								return m.view(keep).getResource(m.attributes[idAttr], fetchOpts);
							}
						})
