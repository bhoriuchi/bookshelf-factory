...
module.exports = function(config) {
	...
	return function(body, fetchOpts, jsonOpts) {
		...
		fetchOpts.idResponse = (fetchOpts.idResponse === true) ? true  : false;
		fetchOpts.verify     = (fetchOpts.verify === false)    ? false : true;
		...
		jsonOpts.omitHref    = (jsonOpts.omitHref === true)     ? true  : false;
		...
		payloadOpts.ignoreInvalid = _.has(fetchOpts, _FOPT.ignoreInvalid) ?
				                    fetchOpts.ignoreInvalid : false;
		
		
		// set the managed check after evaluating the fetch options
		payloadOpts._managedCheck = _.has(fetchOpts, _FOPT._managedCheck) ?
