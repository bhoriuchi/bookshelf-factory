...
module.exports = function(config) {
	...
	return function(body, fetchOpts, jsonOpts) {
		...
		// clear out any depth or circular values from the feetchOpts
		...
		_self.results = u.resolveInput(null, _self).then(function(results) {
			...
			.preparePayload(tableName, body, payloadOpts)
			.then(function(pkg) {
				...
				if (pkg.valid) {
					...
					if (fetchOpts.hasOwnProperty(_FOPT.transacting)) {
						_op = utils.save(opts)(fetchOpts.transacting);
					}
					// check for existing transaction
					else if (_self.transaction) {
						_op = utils.save(opts)(_self.transaction);
					}
					// otherwise create a new transaction and call save
					else {
						_op = Bookshelf.transaction(utils.save(opts));
					}
					...
					.caught(function(e) {
						...
						err = u.newErr(
							e.errno,
							'An SQL error was thrown',
							e.code,
							[e.message, 'thrown from saveResource']
						);
