...
module.exports = function(config) {
	...
	return function(obj, fetchOpts, jsonOpts) {
		...
		var parent  = payload.payload.parent;
		var child   = payload.payload.child;
		...
		if (payload.passed) {
			...
			_.forEach(payload.payload, function(pl, key) {
				...
				_.forEach(pl.payload, function(value, key) {
					...
					var relMatch = util.getCommonPropValue(Object.keys(schema[pl.table][key]), relations);
			...
			// check for updating method
			
			if (method === constants.methods.update) {
				
				if (child.payload.hasOwnProperty('published') &&
						util.toBoolean(child.payload.published) === true) {
			...
			return Bookshelf.transaction(function(t) {
				...
				.forge(where)
				.save(parent.payload, {transacting: t})
				.tap(function(model) {
					parentModel = model;
				...
				.then(function(model) {
					...
					if (payload.payload.hasOwnProperty('child')) {
						...
						return models[child.table]
						.forge()
						.save(child.payload, {transacting: t})
						.tap(function(model){
							
							// update the fetch options with the version
							var modelVersion = model.attributes.version || 0;
							_.merge(fetchOpts, {version: modelVersion});
							
							
							// update relations
							return relutil.updateRelations(child.relations, model, t);
						});
					}
