		...
		return _ts.then(function(sysTime) {
			...
			if (pkg.valid) {
				...
				if (fetchOpts.hasOwnProperty('transacting')) {
					_op = utils.save(pkg.payloads, models, fetchOpts, jsonOpts, saveOpts)(fetchOpts.transacting);
				}
				
				// otherwise create a new transaction and call save
				else {
					_op = Bookshelf.transaction(utils.save(pkg.payloads, models, fetchOpts, jsonOpts, saveOpts));
				}
