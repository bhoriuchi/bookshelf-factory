...
module.exports = function(config) {
	...
	return function(fetchOpts, jsonOpts) {
		...
		if (v) {
			...
			// get the system time
			_parent = util.getTime().then(function(sysTime) {
				...
				return _self.query(function(qb) {
					...
					_verFilter = util.getVersionFilter(v, _timestamp, fetchOpts);
					
					// update the query builder with the join and search
					qb.joinRaw(util.parse(joinSQL, v, v, _verFilter, tableName, idAttr))
