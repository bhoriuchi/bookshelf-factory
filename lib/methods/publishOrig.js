...
module.exports = function(config) {
	...
	return function(id, publish) {
		...
		if (id === null) {
			return utils.util.wrapPromise(_.merge(_STAT.BAD_REQUEST, {details: 'missing id for resource'}));
		}
		...
		return ParentModel.forge(where).fetch().then(function(results) {
			...
			if (publish) {
								...
								if (results) {
									...
									_.forEach(devPayload, function(value, key) {
										...
										if (value === null) {
											...
											if (hasRel !== null) {
												var rel_idAttr = utils.schema.getIdAttribute(schema[colSchema[hasRel]]);
												
												if (hasRel === _REL.belongsToMany || hasRel === _REL.hasMany) {
									...
									if (typeof (devPayload) === _JTYP.object && devPayload !== null) {
				...
				.caught(function(e) {
					return utils.util.wrapPromise(_.merge(_STAT.SQL_ERROR, {error: e}));
				});
			}
		});
	};
