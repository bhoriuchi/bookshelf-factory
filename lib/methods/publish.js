	...
	return function(id, publish) {
		...
		return ParentModel.forge(where).fetch().then(function(results) {
			...
			if (publish) {
				...
				.raw('select ' + util.getUTCSQL(dbType) + ' as now_time')
				.then(function(result) {
					
					
					// validate that a time was obtained
					if (Array.isArray(result) && result.length > 0) {

						var now = _.pluck(result[0], 'now_time');
						
						if (Array.isArray(now) && now.length > 0 && typeof(now[0]) === 'object') {
							
							//console.log('TS:',now[0].toString().split('GMT')[0] + 'GMT');
							// return the date. bookshelf.js seems to add local timezone
							// information to utc dates which causes them to display an
							// incorrect timestamp. as a workaround, strip the timezone
							// of everything to the right of GMT
							return now[0].toString().split('GMT')[0] + 'GMT';
						}
					}
				...
				.then(function() {
					...
					var details = {
						details: 'the version was successfully published',
						version: new_version
					};
					
					return util.statusPromise(_.merge(STATUS.OK, details));
				})
				.caught(function(e) {
