...
module.exports = function(config) {
	...
	// return the function
	return function(pagination, req) {
		
		var _self = this;
		_self._pagination = pagination;
		...
		_self.results = u.resolveInput(null, _self).then(function(results) {
			...
			if (req) {
				...
				if (!_self._href) {
					...
					if (_.has(req, 'headers.host') && _.has(req, 'connection')) {
						...
						_self._href = protocol + req.headers.host;
					}
				}
				...
				if (req.params && pagination.input) {
					...
					if (p.hasOwnProperty(op)) {
						...
						if (typeof(p[op]) === _JTYP.string) {
							...
							for (var j = 0; j < ordr.length; j++) {
								...
								if (ordr[j].length < 2 || (ordr[j][1] !== 'asc' && ordr[j][1] !== 'desc')) {
									ordr[j][1] = dd;
								}
						...
						// check that the order parameter is an array
							...
							_.forEach(p[op], function(o) {
								...
								if (typeof(o) === _JTYP.string) {
									...
									if (ordr.length < 2 || (ordr[1] !== 'asc' && ordr[1] !== 'desc')) {
										ordr[1] = dd;
									}
									...
									if (o[oc]) {
										...
										if (isNaN(o[oc]) && typeof(o[oc]) === _JTYP.string) {
											...
											obj = {};
											obj.field = o[oc];
											obj.direction = (o[od] === 'asc' || o[od] === 'desc') ? o[od] : dd;
											order.push(obj);
										}
					...
					if(!_self._limit) {
						...
						_self._limit = (p.hasOwnProperty(lp) &&
							       !isNaN(p[lp]) &&
							       parseInt(p[lp], 10) <= lm) ?
							       parseInt(p[lp], 10) : ld;
					}
					
					// check for offset/page
					if (!_self._offset) {
						
						// if the request has a page property
						if (p.hasOwnProperty(pp)) {
							
							var curPage   = !isNaN(p[pp]) ? parseInt(p[pp], 10) : pd;
							_self._offset = _self._limit * curPage - 1;
						...
						else if (p.hasOwnProperty(tp)) {
							
							_self._offset = !isNaN(p[tp]) ? parseInt(p[tp], 10) : td;
						}
					}
				}
				...
				_self._limit      = _self._limit  || limit;
				_self._offset     = _self._offset || offset;
				...
				// set options
				if (search.length > 0) {
					_self.search(search, searchable);
				}
				if (order.length > 0) {
					_self.order(order);
				}
			...
			// if no request, return the results
		...
		_self._lastChain = {
			method: config.statics.methods.paginate
		};
