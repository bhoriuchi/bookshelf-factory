...
module.exports = function(config) {
	...
	return function(pagination, req) {
		...
		var _self = this;
		_self._var.pagination = pagination;
			...
			if (req) {
				...
				if (!_self._var.href) {
					...
					if (_.has(req, 'headers.host') && _.has(req, 'connection')) {
						...
						_self._var.href = protocol + req.headers.host;
					}
				}
				...
				if (req.params && pagination.input) {
					...
					if(!_self._var.limit) {
						...
						_self._var.limit = (p.hasOwnProperty(lp) &&
							       !isNaN(p[lp]) &&
							       parseInt(p[lp], 10) <= lm) ?
							       parseInt(p[lp], 10) : ld;
					}
					...
					if (!_self._var.offset) {
						...
						if (p.hasOwnProperty(pp) && pagination.usePages === true) {
							...
							_self._var.offset = (_self._limit * curPage) - 1;
						}
						
						// if the request has a start property
						else if (p.hasOwnProperty(tp)) {
							
							_self._var.offset = !isNaN(p[tp]) ? parseInt(p[tp], 10) : td;
						}
						
						// throw an error if the offset is less than 0
						if (_self._var.offset < 0) {
				...
				_self._var.limit  = _self._var.limit  || limit;
				_self._var.offset = _self._var.offset || offset;
