...
// Description: unpublish function
//
						...
						if (!result) {
							throw u.newErr(
								...
								[tableName + ' with ID ' + id + ' was not found',
								 'thrown from unpublish']
							);
						}
						
						// check if already published
						if (!u.toBoolean(result.attributes.published)) {
							throw u.newErr(
								_STAT.BAD_REQUEST.code,
								_ERR.COULD_NOT_UPDATE.detail,
								_ERR.COULD_NOT_UPDATE.code,
								[tableName + ' with ID ' + id + ' is already unpublished',
								 'thrown from unpublish']
							);
						}
						...
						.then(function(save) {
							...
							if (!save) {
								throw u.newErr(
									...
									_ERR.COULD_NOT_UPDATE.detail,
									_ERR.COULD_NOT_UPDATE.code,
									[tableName + ' with ID ' + id + ' could not be unpublished',
									 'thrown from unpublish']
								);
							}
