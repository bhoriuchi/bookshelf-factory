		...
		return function(t) {
			...
			return promise.map(ids, function(id) {
				...
				return pModel.forge(where).fetch({transacting: t}).then(function(result) {
					...
					// check for active, if the resource is not active it cannot be
					// published
					if (!u.toBoolean(result.attributes.active)) {
						throw u.newErr(
							_STAT.NOT_FOUND.code,
							_ERR.NOT_ACTIVE.detail,
							_ERR.NOT_ACTIVE.code,
							[tableName + ' with ID ' + id + ' is not active, activate it before publishing',
							 'thrown from publish']
						);
					}
							...
							.then(function(result) {
								...
								if (result) {
									...
									.then(function(updatedRels) {
										...
										// check for don't set valid to option
										if (fetchOpts._dontSetValidTo !== true) {
								...
								.then(function(result) {
									...
									.then(function(rels) {
										...
										return promise.map(rels, function(rel) {
											...
											return utils.model.isDraftDiff(rel, t, models)
											.then(function(diff) {
