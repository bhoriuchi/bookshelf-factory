...
module.exports = function(config) {
	...
	return function(opts) {
		...
		// TODO need to re-arrange the get code to support use_current and overrides
			...
			if (v) {
				...
				_parent = utils.sql.getTime({transacting: t}).then(function(sysTime) {
					...
					return _self.query(function(qb) {
						...
						// as a single table
						var joinSQL = parse(
							'inner join (select `%s`.* from `%s` ' +
			                'where %s) v on `%s`.`%s` = `v`.`' +
			                _VER.child.parent_id + '`',
			                v, v, _verFilter, tableName, idAttr);
						
						// update the search SQL
						var publishedSQL = (fetchOpts._reqPub) ?
