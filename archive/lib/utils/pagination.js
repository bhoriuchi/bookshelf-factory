...
module.exports = function(config) {
	...
	function paginate(opts) {
		...
		// get model variables
		var length      = forgedModel._var.limit;
		var start       = !isNaN(forgedModel._var.offset) ? parseInt(forgedModel._var.offset, 10) : 0;
		var href        = forgedModel._var.href || '';
		var baseURI     = href + path;
		var params      = _.has(forgedModel, '_var.req.params') ? forgedModel._var.req.params : null;
		var fCountSQL   = 'count(distinct `' + forgedModel.tableName + '`.`' + forgedModel.idAttribute +'`) as count';
		...
		return _total.then(function(rTotal) {
			...
			return _qb.select(knex.raw(fCountSQL))
			.transacting(t)
			.then(function(rFiltered) {
				
				// set the results
				if (Array.isArray(rFiltered) && rFiltered.length > 0) {
					
					// set the filtered results
					rFiltered = rFiltered[0].count;
				}
