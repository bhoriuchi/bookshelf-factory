...
module.exports = function(config) {
	...
	return function(opts) {
		...
		return function(t) {
			...
			// set the href
			_self._var.href = (jsonOpts.omitHref === true) ? null : _self._var.href;
			...
			return promise.map(opts.packages, function(pkg) {
				...
				.then(function(model) {
					...
					return utils.relation.syncRelations(models, p.relations, model, t, fetchOpts.verify);
					...
					if (isTemporal) {
						...
						.then(function() {
							...
							.then(function(childModel){
								...
								return utils.relation.syncRelations(models, c.relations, childModel, t, fetchOpts.verify);
							});
						});
					}
				})
			...
			.then(function(model) {
				...
				if (Array.isArray(model) && model.length > 0) {
					...
					return promise.map(model, function(m) {
						...
						// emit an event
						emitter.emit(_EVT.save, m.id, models[m.tableName], _self.version, fetchOpts.version);
						...
						if (fetchOpts.idResponse) {
							return u.newObj(m.idAttribute, m.id);
						}
						else {
							// save the model
