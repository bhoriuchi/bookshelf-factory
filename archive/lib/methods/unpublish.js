...
// Description: Main unpublish function.
//
	...
	return function(idList, fetchOpts, jsonOpts) {
		...
		_self.results = u.resolveInput(idList, _self).then(function(results) {
			...
			if (_.has(fetchOpts, _FOPT.transacting)) {
				_op = methodcore.publish(opts, false)(fetchOpts.transacting);
			}
			// check for existing transaction
			else if (_self._var.transaction) {
				_op = methodcore.publish(opts, false)(_self._var.transaction);
			}
			// otherwise create a new transaction
			else {
				_op = Bookshelf.transaction(methodcore.publish(opts, false));
			}
		...
		.caught(function(e) {
			...
			err = u.newErr(
				e.errno,
				'An error was thrown during the unpublish transaction',
				e.code,
				e.message,
				e.stack
			);
